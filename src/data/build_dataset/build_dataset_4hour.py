...
def build(input_file: str, output_file: str, n_snapshots: int = 10):
    ...
    for coin_idx, (coin, markets) in enumerate(raw.items()):
        for m in markets:
            ...
            rows.append({
                **features,
                "duration_minutes": m.get("duration_minutes", 240),
...
if __name__ == "__main__":
    print("building 4-hour dataset...")
    df = build(
        input_file="data/raw/price_history_4hour.json",
        output_file="data/raw/dataset_4hour.csv",
        n_snapshots=10
    )
    print(df.describe())