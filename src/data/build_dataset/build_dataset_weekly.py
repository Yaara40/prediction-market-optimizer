...
def extract_features(market: dict, n_snapshots: int = 14) -> dict | None:
...
def build(input_file: str, output_file: str, n_snapshots: int = 14):
    ...
    for coin_idx, (coin, markets) in enumerate(raw.items()):
        for m in markets:
            ...
            rows.append({
                **features,
                "duration_minutes": m.get("duration_minutes", 10080),
...
if __name__ == "__main__":
    ...
    df = build(
        input_file="data/raw/price_history_weekly.json",
        output_file="data/raw/dataset_weekly.csv",
        n_snapshots=14
    )
    print(df.describe())