...
def extract_features(market: dict) -> dict | None:
    ...
    # pad missing snapshots with None so all rows have same columns
    for i in range(n_snapshots, 10):
        price_snapshots[f"price_t{i}"] = None
...
def build(input_file: str, output_file: str):
    ...
    for coin_idx, (coin, markets) in enumerate(raw.items()):
        for m in markets:
            ...
            rows.append({
                **features,
                "duration_minutes": m.get("duration_minutes", 43200),
...
if __name__ == "__main__":
    ...
    df = build(
        input_file="data/raw/price_history_monthly.json",
        output_file="data/raw/dataset_monthly.csv"
    )
    print(df.describe())