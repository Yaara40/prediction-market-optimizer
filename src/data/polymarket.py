...
if __name__ == "__main__":
    print("--- fetching 12h-7day resolved markets ---")
    resolved = get_resolved_markets_by_timeframe(min_hours=12, max_hours=168)
    total = sum(len(v) for v in resolved.values())
    print(f"\nfound {total} resolved crypto markets (12h-7days)")

    for coin, markets in resolved.items():
        print(f"\n{coin}: {len(markets)} markets")
        for m in markets[:3]:
            print(f"  - {m['question'][:60]} | price: {m.get('lastTradePrice')} | outcome: {m.get('outcomePrices')}")

    with open("data/resolved_markets_12h_7days.json", "w") as f:
        json.dump(resolved, f, indent=2)
    print("\nsaved → data/resolved_markets_12h_7days.json")